...
- `cargo run -p morphogen-cli -- queue-add-feedback-sequence /tmp/morphogen-feedback-queue.json /tmp/source-a-frames /tmp/source-b-frames /tmp/morphogen-feedback-output --carrier-amount 12 --feedback-amount 24 --feedback-mix 0.72 --decay 0.995 --backend metal --flow-source optical-flow` - persist a resumable temporal feedback job with source/cache provenance (`--flow-source` defaults to `optical-flow`; queue jobs serialized before optical flow existed default to `luminance` for backward compatibility).
