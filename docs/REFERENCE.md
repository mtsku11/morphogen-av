...
- `cargo run -p morphogen-cli -- render-granular-mosaic-pool-sequence /tmp/source-a-frames /tmp/source-b-frames /tmp/morphogen-pool-output --grain-size 16 --rearrangement 1 --variation 0 --audio-weight 1 --texture-weight 0 --modulator-rms-cache /tmp/source-a-rms.json --carrier-rms-cache /tmp/source-b-rms.json --frame-rate 24 --grain-cache-dir /tmp/morphogen-pool-cache` - step 6b joint path (`pooled_av_nearest_grain_cpu_v2`, CPU-only): builds a whole-clip temporal grain pool from every Source B frame, each grain carrying its frame-time RMS plus a 2-dim texture descriptor (luma variance + gradient magnitude), and selects per output tile by combined `[mean_color | texture | audio]` distance against Source A's per-tile colour/texture and frame-time RMS query (`--audio-weight` scales the audio dims, `--texture-weight` both texture dims; both default — audio 1, texture 0). `rearrangement` is a cross-frame value blend (0 = carrier, 1 = selected grain). RMS caches are optional but both-or-neither; omit them for colour-only matching across time. Writes/reuses a `grain_pool_descriptors.json` sidecar keyed on the whole carrier set. (The id bumped v1→v2 when texture dims were added, so stale v1 sidecars regenerate.) Selection knobs `--pool-window`, `--anti-repeat-weight`/`--anti-repeat-cooldown`, `--coherence-weight`/`--coherence-reach`, `--spatial-coherence-weight`, and the centroid (k=2) STFT caches are all available here and on `queue-add-granular-mosaic-pool-sequence`.
...
- `cargo run -p morphogen-cli -- render-fluid-advect-two-source-sequence /tmp/source-a-frames /tmp/source-b-frames /tmp/morphogen-fluid-output --advect 2 --reinject 0.08 --frames 120 --backend metal` - render Source A's temporal optical flow advecting Source B's continuous dye. Frame zero is B verbatim; the Metal path is compared with the CPU reference before each frame is written.
- `cargo run -p morphogen-cli -- render-optical-flow-advect-sequence /tmp/source-a-frames /tmp/morphogen-self-fluid-output --advect 2 --reinject 0.08 --frames 120 --backend metal` - run the self-driven variant, where a source's measured motion advects its own held dye. It shares the parity-gated two-source Metal kernel.
- `cargo run -p morphogen-cli -- queue-add-fluid-advect-sequence /tmp/morphogen-fluid-queue.json /tmp/source-frames /tmp/morphogen-fluid-output --frames 120 --frame-rate 24 --advect 12 --reinject 0.05 --backend metal && cargo run -p morphogen-cli -- queue-run-fluid-advect-sequence /tmp/morphogen-fluid-queue.json` - persist and run the procedural faux-fluid dye advection as a ProRes-ready queue bundle with manifest/checkpoint metadata.
- `cargo run -p morphogen-cli -- queue-add-fluid-advect-two-source-sequence /tmp/morphogen-fluid2-queue.json /tmp/source-a-frames /tmp/source-b-frames /tmp/morphogen-fluid2-output --frames 120 --frame-rate 24 --advect 1 --reinject 0.08 --backend metal && cargo run -p morphogen-cli -- queue-run-fluid-advect-two-source-sequence /tmp/morphogen-fluid2-queue.json` - persist and run Source A optical flow advecting Source B's dye; the queue manifest records the `fluid_advect_two_source_cpu_v1` contract and backend.
- `cargo run -p morphogen-cli -- queue-add-optical-flow-advect-sequence /tmp/morphogen-self-fluid-queue.json /tmp/source-frames /tmp/morphogen-self-fluid-output --frames 120 --frame-rate 24 --advect 1 --reinject 0.08 --backend metal && cargo run -p morphogen-cli -- queue-run-optical-flow-advect-sequence /tmp/morphogen-self-fluid-queue.json` - persist and run the self-flow advection variant where the source's own measured motion advects its dye.
- `cargo run -p morphogen-cli -- queue-add-field-particles-sequence /tmp/morphogen-particles-queue.json /tmp/source-frames /tmp/morphogen-particles-output --frames 120 --frame-rate 24 --spacing 8 --particle-size 8 --advect 6 --live-colour --backend metal && cargo run -p morphogen-cli -- queue-run-field-particles-sequence /tmp/morphogen-particles-queue.json` - persist and run the discrete particle carrier with optional live colour, manifest/checkpoint metadata, and parity-gated Metal splatting.
